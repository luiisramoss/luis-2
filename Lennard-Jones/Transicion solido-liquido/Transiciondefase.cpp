...
int main() {
   ...
   //Inicializar fuerzas
    ...
    if ( (k >= 20.000 && k < 20.000 + dt) ||
     (k >= 30.000 && k < 30.000 + dt) ||
     (k >= 35.000 && k < 35.000 + dt) ||
     (k >= 45.000 && k < 45.000 + dt) )
    {
    for (int i = 0; i < N; ++i) {
        vx[i] *= 1.5;
        vy[i] *= 1.5;
    }
    cout << "Velocidades reescaladas en t = " << k << endl;
    }
        ...
        if (k >= 70.0 && k <= 100.0) {
          ...
          // Fluctuaciones cuadráticas de la partícula 0
        double dx = x[0] - x0;
        double dy = y[0] - y0;
        aplicarCondicionContornoDistancia(dx, L);
        aplicarCondicionContornoDistancia(dy, L);
        ...
        fluct << k << ", " << dr2 << ", " << temp_inst << endl; // Guardar tiempo, fluctuación, temperatura
}
